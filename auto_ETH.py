...
while True:
    try:
        ...
        if start_time < now < end_time_model:
            up_yn, X_train, y_test, preds, pred_proba, down_yn, y_test1, preds1, pred_proba1 = Light_GBM("KRW-ETH")
        ...
        if end_time_model < now < end_time:
            ...
            low_price = get_low_price("KRW-ETH")
            krw = get_balance("KRW")
            order_cnt = round(krw / target_price, 4) 
...
#                buy_result = upbit.buy_market_order("KRW-ETH", krw*0.9995)
                ...
                buy_result = upbit.buy_limit_order("KRW-ETH", buy_price, round(order_cnt*0.999,3))
...
#                sell_result = upbit.sell_market_order("KRW-ETH", ETH*0.9995)
                sell_result = upbit.sell_limit_order("KRW-ETH", sell_price, ETH*0.9995)
...
#                sell_result_loss = upbit.sell_market_order("KRW-ETH", ETH*0.9995)
                sell_result_loss = upbit.sell_limit_order("KRW-ETH", current_price, ETH*0.9995)
                post_message(myToken,"#stock", "ETH sell : " +str(sell_result_loss))
            if avg_buy_price > 0 and up_yn == 0 and down_yn == 1:
#                sell_result = upbit.sell_market_order("KRW-ETH", ETH*0.9995)
                uuid = sell_result['uuid'] # 주문번호 얻기
                sell_result_exp = upbit.sell_limit_order("KRW-ETH", current_price, ETH*0.9995)
                post_message(myToken,"#stock", "ETH sell : " +str(sell_result_exp))
            # if end_time1 < now < end_time2 and ETH > 0.003:
            #     sell_result = upbit.sell_limit_order("KRW-ETH", current_price, ETH*0.9995)
